...
function game(){ // 掷筛到动作完成
    ...
    {
        var suolong=Math.ceil(Math.random()*6);
        showMsgbox("Joker的助战伙伴是索隆！他使得其他玩家失去"+suolong*100+"的财产")
        for (var p=0;p<=playerNumber + npcNumber - 1;p++)
            {
                if (p!=s)
                    var person1=players[p];
                person1.money-=suolong*100;
            }
    }
    ...
    {
        showMsgbox("Catwoman的助阵伙伴是山治！他使得其他玩家失去300")
        for (var p=0;p<=playerNumber + npcNumber - 1;p++)
            {
                if (p!=s){
                    var person1=players[p];
