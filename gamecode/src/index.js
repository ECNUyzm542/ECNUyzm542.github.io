...
function game(){ // 掷筛到动作完成
        ...
        if (person.name=='Joker')
    {
        ...
        if (panding()<=26)
            {
                showMsgbox("索隆使得其他玩家失去100分的财产;<br>索隆判定成功，使得其主人获得名下地产数*100的财产");
                places.forEach(place => { 
                ...
                showMsgbox("索隆使得其他玩家失去100分的财产")
            }
    }
    else if (person.name=='Batman')
    {
        var lufei=Math.ceil(Math.random()*6);
        showMsgbox("Batman的助战伙伴是路飞！他使得Batman获得了"+lufei*100+"的财产")
        person.money+=lufei*100;
    }
    else if (person.name=='Superman')
    {
        showMsgbox("Superman的助阵伙伴是娜美！她使得Superman获得了300")
        person.money+=300;
    }
    else if (person.name=='Catwoman')
    {
        for (var p=0;p<=playerNumber + npcNumber - 1;p++)
            {
                    var person1=players[p];
        ...
        if (panding()<=52)
        {
            ...
            person1.stop+=1;
            showMsgbox("山治获得了场上剩余玩家数*100的财产;<br>山治使得玩家"+shanzhi+"休息一天");
        }
            else
            showMsgbox("山治获得了场上剩余玩家数*100的财产")
        }
        else{
            showMsgbox("山治获得了场上剩余玩家数*100的财产");
        }
    ...
    else if (person.name=='Harley Quinn')
    {
        var buluke=200;
        for (var p=0;p<=playerNumber + npcNumber - 1;p++)
            {
             ...
             person.money+=buluke
        showMsgbox("Harley Quinn的助阵伙伴是布鲁克！他使得Harley Quinn获得了"+buluke+"的财产")
    }
    else if (person.name=='Robin')
    {
        ...
        person.money+=150
        showMsgbox("Robin的助战伙伴是罗宾！她夺取了其他玩家的50并交给了Robin")
    }
    else if (person.name=='Green Lantern')
    {
        var wusuopu=Math.floor(Math.random()*(playerNumber+npcNumber))
        var person1=players[wusuopu];
        if (person1.money>0){
            person1.money-=200
            showMsgbox("Green Lantern的助战伙伴是乌索普，他使得"+person1.name+"失去了200")
        }
        else{
            person.money+=200
            showMsgbox("Green Lantern的助战伙伴是乌索普，他使得Green Lantern获得了200")
}
    }
